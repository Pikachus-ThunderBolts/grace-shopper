...
async function getReviewByCustomerUserId(customerUserId) {
    try {
        ...
        console.error("There was an error getting the review by customer User Id", error);
        throw error;
    }
}
...
async function updateReview({id, title, review, customerUserId, productId, guestId}) {
    try {
        const {
            rows: [reviews],
        } = await client.query(`
